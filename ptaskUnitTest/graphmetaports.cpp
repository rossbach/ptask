...
extern char g_szCompilerOutput[];
extern int COMPILER_BUF_SIZE;

int run_metaports_task(	
	char * szfile,
	char * szshader,
	int rows,
	int cols,
	...
	) 
{
	...
	DatablockTemplate * pDataTemplate	= PTask::Runtime::GetDatablockTemplate("dbV1_float", stride, n+1, 1, 1);
	DatablockTemplate * pODataTemplate	= PTask::Runtime::GetDatablockTemplate("dbV1_float", stride, n, 1, 1);
	...
	if(NULL == (pKernel = PTask::Runtime::GetCompiledKernel(szfile, szshader, g_szCompilerOutput, COMPILER_BUF_SIZE))) {
        std::cout << g_szCompilerOutput << std::endl;
        return 1;
    }

    float * pInitialValue = uniform_vector(0.0, 1);
    pDataTemplate->SetInitialValue(pInitialValue, sizeof(float), 1);
	...
	pAplusBInputPorts[2]	= PTask::Runtime::CreatePort(STICKY_PORT, pParmTemplate, uiUidCounter++, "N-parm", 3);
	pAplusBInputPorts[3]	= PTask::Runtime::CreatePort(META_PORT, pParmTemplate, uiUidCounter++, "N-meta", uiOutPortIndex);
	pAplusBOutputPorts[0]	= PTask::Runtime::CreatePort(OUTPUT_PORT, pDataTemplate, uiUidCounter++, "B(out)", 1);
